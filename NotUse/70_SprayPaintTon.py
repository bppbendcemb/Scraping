...
file_path = os.path.join(folder_Output, 'SprayPaintTon2.csv')    
...
for index, row in spraypaint_df.iterrows():
    ...
    if result > 0:
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, GETDATE())
        """
        cursor.execute(sql, row['uniqueid'], row['yr'], 70,  # เปลี่ยน row['id'] เป็น 70
